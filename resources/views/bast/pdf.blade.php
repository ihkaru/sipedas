...
<head>
    @php
        ...
        // KARENA DATA SUDAH DIFILTER, KITA HANYA PERLU GROUPING BERDASARKAN MITRA
        // Ini akan membuat satu halaman BAST per mitra untuk kegiatan yang dipilih.
...
<body>
    @foreach ($alokasiPerMitra as $alokasiMitra)
        @php
            // Ambil data pertama dari koleksi mitra ini, karena data mitra dan PPK akan sama untuk satu BAST.
            $firstAlokasi = $alokasiMitra->first();
            // Jika tidak ada data, lewati untuk menghindari error.
            if (!$firstAlokasi) {
                continue;
            }
        ...
        {{-- Gunakan pagebreak jika ini bukan iterasi pertama --}}
        @if (!$loop->first)
        ...
        <div style="display: flex;">
            <div style="max-width: 600px; display: flex">
                <div>
                    <span>
                        <img width=97 height=69 src="{{ asset('SPPD_files/Image_001.png') }}"
                ...
                <div
                  ...
                  ">
                    ...
                    <p class="s2" style="text-indent: 0pt; text-align: left; padding-top:10pt">
                        <a href="https://mempawahkab.bps.go.id/" class="s3" target="_blank">Jln. Raden Kusno No
                            59 Mempawah 78912 Telp (+62561)691049 Fax (+62561)
                            6695439 Homepage: </a><span
                            style="
                        color: #00f;
                        font-family: Calibri, sans-serif;
                        font-style: normal;
                        ...
                        <a href="mailto:bps6104@bps.go.id" class="s3" target="_blank">E-mail:
                            bps6104@bps.go.id</a>
                    </p>
                </div>
            </div>
        ...
        <table style="padding-left: 50px;">
            ...
            <tr style="vertical-align: bottom;">
                ...
                <td>
                    <p>Kecamatan {{ ucwords(strtolower($firstAlokasi->mitra->kecamatanName)) }}, Desa/Kelurahan
                        {{ ucwords(strtolower($firstAlokasi->mitra->desaName)) }}</p>
                </td>
            </tr>
        </table>
        <br>
        ...
        <h3 style="padding-top: 6pt; padding-left: 58pt; text-indent: -18pt; line-height: 150%; text-align: left;">
            ...
            <span class="p">telah menyelesaikan pekerjaan
                {{-- LOOPING DETAIL PEKERJAAN DARI KOLEKSI MITRA INI --}}
                @foreach ($alokasiMitra as $h)
                    {{ ucwords(strtolower($h->honor->jenis_honor)) }} {{ $h->honor->kegiatanManmit->nama }}
                    sebanyak
                    {{ (int) $h->target_per_satuan_honor . ' ' . $h->honor->satuan_honor }}@if (!$loop->last)
                        ,
                    @elseif(count($alokasiMitra) > 1 && $loop->remaining == 1)
                        dan
                    @endif
                @endforeach
            </span>
        </h3>
        <h3 style="padding-left: 58pt; text-indent: -18pt; line-height: 150%; text-align: left;">
            <span class="p">b. </span>
            PIHAK PERTAMA
            <span class="p">telah memeriksa dan menerima dengan baik hasil pekerjaan di Badan Pusat Statistik
                Kabupaten Mempawah dari PIHAK KEDUA.</span>
        </h3>

        <p style="text-indent: 0pt; text-align: left"><br /></p>
        <p style="padding-left: 5pt; text-indent: 0pt; line-height: 151%; text-align: left;">
            Demikian Berita Acara Serah Terima Pekerjaan ini dibuat agar dipergunakan sebagaimana mestinya.
        </p>
        <p style="text-indent: 0pt; text-align: left"><br /></p>
